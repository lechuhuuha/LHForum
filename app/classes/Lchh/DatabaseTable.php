...
class DatabaseTable
{
    ...
    public function findAllWithLimit($offset = 1, $no_of_records)
    {
        $result = $this->query('SELECT * FROM ' . $this->table . ' LIMIT ' . $no_of_records . ' OFFSET ' . $offset);
        return ($result->fetchAll(\PDO::FETCH_CLASS, $this->className, $this->constructorArgs));
    }
    /**
    ...
    /**
     * If its has id then update if not it's  insert 
     * @param mixed $record
     * @return mixed $entity
     */
